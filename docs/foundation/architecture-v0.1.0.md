...
策略层
  ...
  │   **元认知触发评估注记（外部理念吸收 0.0.48；外部实证：PAPER-13 MemGen 记忆触发器）**：MemGen 以 RL 训练的 LoRA 触发器持续监视推理状态、在句子级标点处判定是否唤起记忆（主模型零微调；ALFWorld +44.64%，第三方口径）——「何时需要记忆」的元认知判定可训练、可稀疏化，是预测器「下一步该回忆什么」职责的外部实证。Kairos 评估：触发判定升级为学习式（§10.14 RL 路径）的候选——触发决策须可审计（何时触发、为何触发可解释），触发策略行为受宪法边界约束；v0.1.0 保持规则式，学习式触发为 v1.1 评估项。
