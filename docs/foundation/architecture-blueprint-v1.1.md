...
**设计要点**：通过 `POST /v1/facts`（见 api-spec §10）将 (subject, predicate, object) 三元组直接写入实体知识图谱和关系索引，绕过 LLM 提取管道。请求体含事实数组（含置信度/来源/证据/时间戳）和选项（跳过 LLM 提取、直接索引、关联记忆 ID）。
...
│   - **管理接口**：通过边类型管理 API（见 api-spec §11）注册新边类型签名、查询已注册签名、更新合法组合（不影响已有边——仅对新写入生效）。
...
#### 技能管理系统（Skill Management System）
  ...
  │     archived 技能不参与 find_skills 检索，仅通过技能列表 API 历史查询可见（见 api-spec §12 GET /v1/skills）。
  │   - `active/deprecated → superseded`：新技能通过技能管理 API 标记替代关系（见 api-spec §12 POST /v1/skills/{id}/supersede），旧技能写入 `superseded_by` 字段。
  │   - 复活路径：archived 技能可通过技能管理 API 重新激活（见 api-spec §12 POST /v1/skills/{id}/reactivate）为 experimental（置信度重置为 0.5），重新进入验证周期。
...
> **v0.1.0 已采用（决策 D-05）**：本节的 v0.1.0 采用版已迁入 [architecture-v0.1.0.md](architecture-v0.1.0.md) §5.5 作为正式交付能力，该处为权威定义；本蓝图保留为 v1.1+ 演进详版。下文 §5.3 / §5.4 / §5.6 对其的引用改指主架构文档 §5.5。
...
**探索产物置信度带**（B-3 决策）：探索产出的记忆（标记 `provenance=exploration`）在影子副本中使用降权累积——初始置信度设为基础阈值的 30%（而非常规记忆的基础值 50%），每成功验证一次提升一档（30%→50%→70%→95%→可合并回主副本）。在达到 70% 前，探索产物不参与激活权重计算的帕累托前沿输出（不进入候选集），仅保留在影子副本供验证；达到 70% 后可进入候选集但权重仍受衰减约束，达到 95% 后转为常规记忆。此降权累积与 S-14（语境自指禁令）一致——使用权重陡升触发差异检验的规则对探索产物同样适用。降权累积的衰减周期和验证阈值见配置文档
...
- **渲染引擎**：Mermaid.js 的 `graph` 类型（支持 LR/TD 布局方向）+ Canvas 渲染后端。生成的 Mermaid 图通过 Kairos 内嵌的 Web 仪表盘展示，或通过图谱可视化 API（见 api-spec §16 GET /v1/graph/render）导出为 SVG/PNG
- **节点样式映射**：
