...
> **注（口径桥接）**：本矩阵按**原 P 编号**计——C-26（软原则，原 P2）与 C-25/C-27~C-29 构成「五硬一软」（C-30 原 P6 不在此组：其行级状态为「已知超限（非受控偏离）」且无 ✅ 标记，见矩阵 C-30 行）。**认知基础 §2.2 按合并后计「三硬一软」**——硬约束 1 = 原 P3+P6、硬约束 2 = 原 P1+P4、硬约束 3 = 原 P5、软原则 = 原 P2。两口径为同一原则集的不同计数方式，**以认知基础 §2.2 合并口径为权威**（「三硬一软」非旧标签，与「五硬一软」并存互译）。
