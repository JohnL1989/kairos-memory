...
> **状态声明**：本文列出的能力总数为设计就绪状态下的近似值（**168 项，含 43 核心 + 125 扩展**；最终计数见文末「最终计数」表）。截至 v0.1.0，全系统尚无运行代码（见 changelog/project-plan）。各功能的状态标记为`冻结`——表示**设计已固化、架构规划已完成，但代码尚未编写（非"已实现"）**。
...
| 0.0.2 | 2026-08-04 | 全库深度审计修复：引用章节修正、R-09 计数表修正。 |
| 0.0.3 | 2026-08-04 | 全库深度审计修复：P3-11~13 来源补注（0.0.14 补登——与 requirements-baseline 0.0.3 条目同批次，changelog 0.0.3 未逐条登记本文档）。 |
| 0.0.4~0.0.9 | 2026-08-04 | （合并占位：changelog 0.0.4~0.0.9 批次的变更未逐条登记于本文档，见 [changelog.md](../governance/changelog.md) 全景） |
| 0.0.10 | 2026-08-04 | 第二轮全库深度审计修复（changelog 0.0.10）：SF-11/12/13 移回扩展功能表；删除两张历史分类快照表；PM-01 意图契约口径；裸引用补链接。 |
| 0.0.11 | 2026-08-04 | 开发就绪度修复批次：竖切表补 R-18 三信号混合检索注记、关系类型枚举对齐 data-model。 |
| 0.0.14 | 2026-08-05 | 开发就绪度审计修复批次（changelog 0.0.14）：F-03 复兴触发口径勘误（潜伏势能重估/外部校准，非检索触发）；版本记录补 0.0.3 条目。 |
| 0.0.16 | 2026-08-05 | 开发就绪度审计修复批次（changelog 0.0.16，建议五落地）：M-21 Saga 叙事线补 v0.1.0 子集注记（创建/添加/检索/完结四操作，summarize 与自动聚合为 v1.1 目标）。 |
