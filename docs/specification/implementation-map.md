...
> **已对齐**（2026-08-03 核定，2026-08-04 复核，2026-08-13 参数计数 380→381 同步）：表数 **57** 与 [data-model.md](data-model.md) 一致（该文档现有 65 个 `###` 条目——57 物理表 = 65 减 `retrieval_enhancement_config`（配置参数清单）减 8.12/8.17 两节标题减 13.1~13.5 五节；原「58 个 ### 条目」口径经勘误）；端点数 **85** 与 [api-spec.md](api-spec.md) 一致——口径为 `/v1` 业务端点（注册 `archive`/`restore` 后 78→80，新增 `health/calibration`、`health/memory-pressure` 与叙事线三端点后 80→85），另有 3 个无 `/v1` 前缀端点（`GET /health` 探针、`GET /audit/compression`、`GET /audit/compression/summary`）不计入（物理总数 88，见 [api-spec.md](api-spec.md) §版本记录）；本文档登记 **70** 个组件代码路径（按下方各表第二列 `src/` 路径行统计）；参数总数 **381**（configuration 正文 227 + 附录 A 154，见 [configuration.md](../ops/configuration.md)；批次口径同步）。
...
> **⚠ 代码状态声明**：竖切已交付组件（access/storage/sovereignty/supervision/events 等，见 [slice-implementation-guide.md](../development/slice-implementation-guide.md)）按下方映射落地；竖切范围外的 `src/` 路径仍为规划目标，编码启动后按此映射创建模块。
...
| 记忆状态机 | `src/storage/memory_state_machine.py` | Active→Stale→Archived→Suppressed→Superseded 五态平级（active/stale/archived/suppressed/superseded，无子态；架构 §5.2 五态口径；v1.1 目标含 CANDIDATING 候选阶段）+ 状态变更跟踪 |
...
| 配置加载 | `src/config.py` | 环境变量/配置文件加载，381 项参数（configuration 正文 227 + 附录 A 154，与 configuration 一致） |
...
| 0.0.2~0.0.10 | 2026-08-04 | （合并占位：changelog 0.0.2~0.0.10 批次的变更未逐条登记于本文档，见 [changelog.md](../governance/changelog.md) 全景） |
