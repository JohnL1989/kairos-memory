...
| 同一记忆并发检索 | 无锁，顺序执行 | — |
| 遗忘与检索并发 | 遗忘不阻塞检索（标记后清除，标记态非最终态——已被标记为遗忘的记忆在清除前仍可被检索命中，但返回时标记 `stale=true`）。标记态是遗忘调度器的中间状态，非持久状态机态——持久状态机为五态平级（active/stale/archived/suppressed/superseded，见 [data-model.md](../specification/data-model.md) §1 `status`），其中 suppressed 由定向遗忘操作写入（被抑制路径不可检索，数据仍存在），遗忘调度器清除冷存储记忆后不产生新的持久状态机态 | 已清除的记忆不被返回 |
