...
> **状态声明**：竖切实现已交付，发布流程可按本文 §流程执行；尚未落地的发布基础设施（镜像发布/自动发布管线等）见 [debt-collection.md](../governance/debt-collection.md)。
...
> **⚠ 执行条件**：以下步骤假定 Kairos 已有可运行的代码包——竖切已交付（wheel 已构建，`dist/kairos-0.1.0*`），可执行；镜像发布等未落地基础设施见 [debt-collection.md](../governance/debt-collection.md)，执行对应步骤前以其状态为准。
