...
[claim-implementation-matrix.md](../specification/claim-implementation-matrix.md) 的版本化变更边界规则（原「版本边界」节）为文档治理规则，权威定义如下，矩阵仅保留状态标记与指针：
...
| 0.0.17 | 2026-08-05 | 开发就绪度审计修复批次（changelog 0.0.17，外部建议 R-9 落地）：§5 债务编号分段语义补「前置依赖声明规则」——新债务登记须声明前置依赖并同步关键路径依赖表。 |
...
| 0.0.42 | 2026-08-07 | 0.0.42 文档审计修复批次（changelog 0.0.42）：§6 豁免 2 修订（外部实证标注双轨：参数校准类实证参考基线 + 机制描述类来源代号简注）；新增 §6.1 声明承载矩阵版本边界（自 claim-matrix 剥离）。 |
