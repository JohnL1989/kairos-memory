...
> **批次定位**：本目录承载「B站 AI Agent 记忆系统视频 × Kairos」对照分析批次的全部产出——100 个视频的素材边界声明、逐视频精读笔记、外部理念分诊矩阵、第一性原理对照评审与吸收建议清单。**本批次不修改任何核心设计文档**（foundation/specification 零改动），吸收建议止于「建议态」，是否落库由后续批次决策。
...
| 级别 | 获取途径 | 可信度 | 数量 |
|:--|:--|:--|:--|
| A. B站 AI 字幕（匹配） | `x/player/v2` + Cookie，人工核验中段内容与主题匹配 | 高（个别错字） | 22 |
| B. 本地 whisper 转写 | yt-dlp 下载音频 + faster-whisper（small/int8）本地转写 | 高（转写质量经抽查） | 72（19 精选 + 41 全量 + 12 追加/重试） |
| C. 素材降级 | 无有效语音 / 20 小时串台合集 / 字幕全音乐 / 转写静音 | 不适用（仅元信息） | 6 |
...
|:--|:--|:--|:--|:--|:--|
| VID-01 | BV1zUTn61EY8 | 7个AI记忆系统从夯到拉 | 小行星AI观测站 | 8min | C（字幕全音乐） |
...
| VID-13 | BV1tiuA6mENb | TencentDB Agent Memory v2.0 | 智能体老王 | 4min | C（游戏串台） |
| VID-14 | BV1j3gz6vEQQ | Anthropic 官方 Agent Memory | AI变局 | 28min | A |
| VID-15 | BV1TUM26vEAC | pi-hermes-memory 实战 | 程序员暮闲 | 9min | C（串台） |
| VID-16 | BV1YXVg69EmC | OpenClaw 三大搜索+七家嵌入 | big叔大 | 4min | A |
| VID-17 | BV136oYBWEU2 | 个人 Agent 记忆系统实现 | 小单说AI | 13min | C（硬件串台） |
| VID-18 | BV1XCKL6SEg5 | Agent记忆系统设计 | 赫尔辛根默斯肯人 | 6min | C（串台） |
| VID-19 | BV11RGX6bEdc | OpenClaw 三层架构+Dreaming | big叔大 | 6min | A |
| VID-20 | BV1C8LF62EWa | 9K 星：给 AI Agent 加上长期记忆 | AIlazy俊 | 3min | A |
| VID-21 | BV1V9Lp68Ey1 | TencentDB-Agent-Memory 解析 | 鲲鹏Talk | 5min | C（串台） |
| VID-22 | BV1vFME6eEHP | Memorix 共享记忆 | 老汤的碳基突围 | 7min | C（串台） |
| VID-23 | BV18kDjBxEYR | 某讯 Agent Memory 分层索引 | AGI_Ananas | 5min | C（串台） |
| VID-24 | BV1LVMV6nEMt | 构建Agent长期记忆系统（10P） | 小寒说IT | 127min | A（6P 匹配，3P 串台） |
| VID-25 | BV1xoLD6cEEL | 人脑仿生遗忘系统 | 老纪的技术唠嗑局 | 13min | B |
| VID-26 | BV1F7P9zLErb | mem0 开源框架解析 | 加菲大杂烩 | 40min | C（串台） |
| VID-27 | BV1H8VQ6DEBB | Mem0 极简路线 | 为什么叫QQ | 12min | A |
| VID-28 | BV1oxNZ6uE6G | Memorix 记忆跟着项目走 | AI技术投降派 | 11min | A |
| VID-29 | BV1NyMk6QEKB | AI面试：如何实现长期记忆 | DeepTalk-Pro | 2min | C（串台） |
| VID-30 | BV1FpRaBFENL | 4款记忆方案分清 | Ali厂长 | 5min | A |
| VID-31 | BV1rnM269ErH | OpenClaw 讲透 Agent 记忆 | 小马Bosn | 18min | A |
| VID-32 | BV1tk376oEHn | AutoMem 记忆演化技能 | breezedeus | 29min | C（串台） |
| VID-33 | BV1QwGT6MEKQ | OptMem 一行一段永久记忆 | jeffzhengye | 7min | C（短剧串台） |
| VID-34 | BV1PU3b6pEVP | 记忆写入全链路 | AI架构师Leo | 14min | B |
| VID-35 | BV1HhGV6kEmt | Metis 原生记忆基础模型 | 熊二等兵 | 18min | B |
| VID-36 | BV1P4Mr6LEem | AutoGenetic Memory | openJiuwen | 1min | C（串台） |
| VID-37 | BV1L2gY6jErn | Claude 2026 记忆系统新功能 | AI酷生活 | 3min | A |
| VID-38 | BV1v8PYz1EUt | mem0 代码精读 | 日新月异max | 101min | C（串台） |
| VID-39 | BV1k65X6dExY | Letta/MemGPT 持久记忆 | AI技术投降派 | 5min | A |
| VID-40 | BV18yX6BFEHS | Zep 长期记忆系统 | 郭宏志-老郭 | 14min | C（导航串台） |
| VID-41 | BV1nS7X6hECB | supermemory 长期记忆 | 码上成功的小猴 | 1min | C（硬件串台） |
| VID-42 | BV1JggX6kEbs | 让AI自动进化的记忆系统 | 新书提拉 | 4min | A |
| VID-43 | BV1vkEj6WE3F | 基于Mem0的上下文和记忆管理 | 人月聊IT | 6min | C（串台） |
...
| VID-48 | BV11wga6tEGH | 从零构建生产级 Agent Memory（二） | 老纪的技术唠嗑局 | 5min | C（串台） |
| VID-49 | BV15L3F6pEFN | 从零构建生产级 Agent Memory（三） | 老纪的技术唠嗑局 | 4min | C（串台） |
| VID-50 | BV1AA3i6TEAU | 从零构建生产级 Agent Memory（四） | 老纪的技术唠嗑局 | 4min | C（串台） |
| VID-51 | BV1fp3k6mEJU | 从零构建生产级 Agent Memory（五） | 老纪的技术唠嗑局 | 4min | C（串台） |
| VID-52 | BV145326zEAM | 从零构建生产级 Agent Memory（六） | 老纪的技术唠嗑局 | 4min | A |
| VID-53 | BV1wkMB6qEyN | Google Memory Bank 长期记忆 | AI变局 | 21min | C（串台） |
| VID-54 | BV18G3z6oE4p | Anthropic Memory+Dreaming | AI变局 | 27min | C（串台） |
...
| [papers/](papers/) | 2026 论文深读分析笔记（PAPER-01~09，源自 AI HOT 学术档案 + 用户映射分析） |
...
| 项 | 状态 |
|:--|:--|
| 素材抓取 | ✅ 100 个视频：22 字幕匹配 + 72 whisper 转写 + 6 降级 |
| 逐视频笔记 | ✅ 100/100（102 份，含 2 份降级标注） |
| 分诊矩阵 / 原理评审 / 吸收建议 | 待笔记完成后汇总 |
| 门禁 | 待批次完成后运行 doc-audit.py |
