...
> **容量预算**：单次全量备份体积 ≈ 数据库文件体积（1 万条 ≈ 数十 MB，100 万条 ≈ 2GB 级）；30 天保留总量预算 ≤ 单次体积 × 30 + WAL 7 天 × 日增量 + 升华快照 30 天 × 单次体积。备份目录占用 ≥ 磁盘 75% 时触发清理最旧备份（保留期最短的 WAL 先行），≥ 92% 时停止升华快照并告警（与 §1.4 磁盘告警联动）。
>
